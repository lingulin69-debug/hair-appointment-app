...
function normalizeFirebaseConfig(value: Partial<FirebaseConfig>): Partial<FirebaseConfig> {
  return {
    apiKey: normalizeString(value.apiKey),
    authDomain: normalizeString(value.authDomain),
    projectId: normalizeString(value.projectId),
    storageBucket: normalizeString(value.storageBucket),
    messagingSenderId: normalizeString(value.messagingSenderId),
...
function resolveFirebaseConfig(): FirebaseConfig {
  ...
  if (typeof __firebase_config !== 'undefined') {
    try {
      const parsed = normalizeFirebaseConfig(
        JSON.parse(__firebase_config) as Partial<FirebaseConfig>
      );
