    ...
    <div className="min-h-full bg-[#FCFAF5] p-4 md:p-6">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <h1 className="text-2xl font-bold text-[#4A3B32] md:text-3xl">
              服務與商品
            </h1>
          ...
          {onAddItem && (
            <button
              type="button"
              onClick={onAddItem}
              className={`inline-flex items-center gap-2 rounded-lg bg-[#4A3B32] px-4 py-2 text-white shadow-sm hover:bg-[#5D4A3F] ${interactionMotion.button}`}
            >
              <Plus className="h-4 w-4" />
              <span className="font-medium">新增項目</span>
            </button>
          )}
        </div>
          ...
          {FILTERS.map((filter) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-2 whitespace-nowrap rounded-lg px-4 py-2 font-medium ${interactionMotion.subtleButton} ${
                  isActive
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <h2 className="text-xl font-bold text-[#4A3B32]">{title}</h2>
    ...
    >
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="truncate text-lg font-semibold text-[#4A3B32]">
            {item.name}
          </div>
      ...
      <div className="flex gap-2">
        {onSelectItem && (
          <button
            ...
            className={`flex-1 rounded-lg py-2 font-medium ${interactionMotion.subtleButton} ${
              isSelected
        ...
        {onDeleteItem && (
          <button
            ...
            disabled={isDeleting}
            className={`inline-flex items-center gap-1 rounded-lg border border-[#E5D6C5] px-3 py-2 text-sm font-medium text-[#9A4F44] hover:border-[#C75D4E] hover:bg-[#FFF4F2] disabled:cursor-not-allowed disabled:opacity-60 ${interactionMotion.subtleButton}`}
          >
