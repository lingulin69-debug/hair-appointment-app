...
- **多日排程改版已完成**：保留原本月曆模式，新增 `月曆 / 多日排程` 雙模式；多日排程已改為每個日期欄位自行顯示整點時間，不再保留左側細時間導引，也不再顯示 30 分鐘標示。
- **時間顯示已再調整**：欄內整點時間文字已再放大 3px，讓長輩更容易辨識。
- **日期跳轉已完成客製化**：改為自訂月曆彈窗，非原生 `type=date`，支援清除、今天與月份切換。
- **本機與正式部署已完成**：已建立本機備份 `backup/hair-appointment-app-main-20260427-005153.zip`，並成功部署到 Vercel production，正式網址仍為 `https://hair-appointment-app-nine.vercel.app`。
- **驗證結果**：`npm run test -- src/components/Calendar/Calendar.test.tsx` 通過，`npm run build` 通過。
- **Firebase / Firestore 正式站驗證已完成**：正式站已確認連到 `hair-1b310`；同步按鈕可正常變成 `已同步`；並已做過真實寫入測試，在顧客頁用 UI 新增測試顧客、重新載入後仍能讀回，最後已刪除測試資料並回查剩餘 0 筆。
- **目前判定**：使用者本輪需求已完成，專案可先關閉；若未來再開啟，優先閱讀本檔、`明日接手計畫.md` 與 `我的 AI 大腦/2026-04-27_多日排程整點版_部署備份.md`。
  - `src/hooks/useAppointments.ts`
  - `src/hooks/useStoreItems.ts`
  - `src/hooks/useLeaves.ts`
...
## 18. 2026-05-05 正式站巡檢與可及性修正

- **正式站巡檢已完成**：已逐項檢查主站首頁、日曆雙模式切換、預約統計概況與收入頁、服務與商品、顧客目錄、顧客詳情彈窗、同步按鈕、計算機，以及 `recovery.html`、`import.html`。
- **巡檢結果**：
  - 主流程皆可正常載入與切換。
  - console 未發現 runtime error。
  - network 未見失敗請求；主資源、字體與 Firestore listen 請求皆為 `200`。
- **本輪修正內容**：
  - 補上 5 個表單欄位的 `id` / `name`，消除正式站巡檢時看到的可及性提醒。
  - 修正位置包含：
    - `src/components/Dashboard/Dashboard.tsx` 的日期、金額、分類、備註欄位。
    - `src/components/Client/ClientList.tsx` 的顧客搜尋欄位。
- **本輪驗證方式**：
  - 先用正式站巡檢確認提醒來源。
  - 再於本機補修欄位屬性並重新建置驗證。

## 19. 2026-05-05 時段擴充與收入頁互動巡檢

- **部署版本資訊**：已推送 commit `d14863d`，commit 訊息為 `feat: extend schedule hours and polish revenue form`；正式網址仍為 `https://hair-appointment-app-nine.vercel.app`。
- **本輪主要調整**：
  - 多日排程顯示範圍由 `10:00-24:00` 改為 `08:00-24:00`。
  - 新增預約草稿不再硬塞 `10:00`，一般開窗時會自動帶到最早可用時段。
  - 收入頁手動記帳區新增頁內提示框、`清空` 按鈕，且未填日期/金額前會禁用 `新增記帳`。
- **本機驗證結果**：
  - `npm test -- src/components/Calendar/NewApptModal.test.tsx src/components/Calendar/Calendar.test.tsx`：通過。
  - `npm run build`：通過。
- **正式站更細巡檢結果**：
  - 日曆 `月曆` 模式點一般 `新增預約` 按鈕後，modal 預設時間已為 `08:00`。
  - 日曆 `多日排程` 模式已可直接看到 `08:00 / 09:00 / 21:00 / 22:00 / 23:00` 時段；點指定 slot 開窗時，modal 也會正確帶入對應時間。
  - 收入頁手動記帳在金額空白時維持禁用；輸入金額後會啟用 `新增記帳`；按 `清空` 後會回到禁用狀態。
  - 收入頁切換 `扣帳` 時，分類預設值會自動改成 `其他支出`。
  - 本輪巡檢僅做 UI/互動驗證，未送出新的正式資料。
  - 正式站 console 仍為乾淨，未發現新的 warning 或 error。
